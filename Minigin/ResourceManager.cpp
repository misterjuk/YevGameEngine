...
void yev::ResourceManager::Init(const std::filesystem::path& dataPath)
{
	m_dataPath = dataPath;
...
std::shared_ptr<yev::Font> yev::ResourceManager::LoadFont(const std::string& file, uint8_t size)
{
	const auto fullPath = m_dataPath/file;
	const auto filename = fs::path(fullPath).filename().string();
...
void yev::ResourceManager::UnloadUnusedResources()
{
